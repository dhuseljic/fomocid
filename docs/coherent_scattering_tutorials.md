...
- **Vertical x-z aperture diagnostics**: the multislice/ROI tutorial includes a
  didactic x-z wavefront view through each aperture. It rebuilds the tutorial
  aperture stack, overlays aperture walls plus SiN and magnetic-layer borders,
  and shows amplitude/phase as the field is propagated through the stack. This
  is a visualization aid, not an extra HDF5 output generated by the pipeline.
