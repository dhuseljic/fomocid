...
def focus(pos,neg, roi, mask=1,phase=0, prop_dist=0,dx=0, dy=0, scale=(0,100), experimental_setup={'ccd_dist':18e-2, 'energy':779.5, 'px_size':20e-6}, operation="-", max_prop_dist=10):
    ...
    layout = widgets.Layout(width='90%')
    style = {'description_width': 'initial'}
    slider_prop = widgets.FloatSlider(min=-max_prop_dist, max=max_prop_dist, step=max_prop_dist/100, value=prop_dist, layout=layout,
                                      description='propagation[um]', style=style)
    slider_phase = widgets.FloatSlider(min=-np.pi, max=np.pi, step=0.001, value=phase, layout=layout,
                                       description='phase shift', style=style)
    slider_dx = widgets.FloatSlider(min = -4, max = 4, step = 0.01, value = dx, layout = layout,
...
def focusCDI(pos,neg, roi, mask=1,phase=0, prop_dist=0,dx=0, dy=0, scale=(0,100), experimental_setup={'ccd_dist':18e-2, 'energy':779.5, 'px_size':20e-6}, operation="-", max_prop_dist=10):
    ...
    def p(x, y, fx, fy):
        ...
        if operation== "-":
            ...
            image= np.log(np.abs(image_p/image_n)* np.exp(1j*y))
        elif operation=="**2/**2":
            image= (np.abs(image_p)**2/np.abs(image_n)**2) * np.exp(1j*y)
        elif operation=="-/+":
            image= (image_p-image_n)/(image_p+image_n) * np.exp(1j*y)
...
def filter_HER(im, phi, w=10, sigma=3):
    
    im_filt=np.ones(im.shape)
    
    l=im.shape[0]
    w=10
    center=[im.shape[0]//2,im.shape[0]//2]
    
    phi2=np.deg2rad(phi)
 
    r=np.array([  -l*np.cos(phi2)-w*np.sin(phi2),  -l*np.cos(phi2)+w*np.sin(phi2),  l*np.cos(phi2)+w*np.sin(phi2),  l*np.cos(phi2)-w*np.sin(phi2)  ])
    c=np.array([  -l*np.sin(phi2)+w*np.cos(phi2),  -l*np.sin(phi2)-w*np.cos(phi2),  l*np.sin(phi2)-w*np.cos(phi2),  l*np.sin(phi2)+w*np.cos(phi2)  ])
    ...
    yy,xx=polygon(r, c, shape=im.shape)
    
    im_filt[yy,xx]=0
    
    im_filt=gaussian_filter(im_filt, sigma)
...
def HERALDO(im, phi=0, w=0, sigma=3):
    ...
    df = 1j*(-xx*np.sin(np.deg2rad(phi))+yy*np.cos(np.deg2rad(phi)))
    
    im=fth.reconstruct(im*df*filter_HER(im, phi, w, sigma))

    return im


def filter_HER2(im, phi, w=10, sigma=3):
...
def HERALDO2(im, phi=0, der = 1, w=0, sigma=3):
    ...
    for i in range(len(phi_list)):
        ...
        if der[i]==1:
            df *= 1j*(-xx*np.sin(np.deg2rad(phi2))+yy*np.cos(np.deg2rad(phi2)))
    
    im=fth.reconstruct(im*df*filter_HER2(im, phi, w, sigma))

    return im



def focusHERALDO(pos,neg, phi,der,w, sigma,roi, mask=1,phase=0, prop_dist=0,dx=0, dy=0, scale=(0,100), experimental_setup={'ccd_dist':18e-2, 'energy':779.5, 'px_size':20e-6}, operation="-", max_prop_dist=10):
    '''
    Applies a sub-pixel centering, propagation distance and global phase shift.
    Also plots real,image,abs,angle images while you do it
    INPUT:  pos,neg: array, the shifted and masked holograms
    ...
    fig, axs = plt.subplots(2,2, figsize=(10,10))
    def p(x, y, fx, fy):
        image_p = HERALDO2(fth.propagate(pos, x*1e-6, experimental_setup)* np.exp(1j*y), phi,der, w, sigma)
        image_n = HERALDO2(fth.propagate(neg, x*1e-6, experimental_setup)* np.exp(1j*y), phi,der, w, sigma)
        ...
        if operation== "-":
            ...
            image,_=fth.load_both(pos,neg,True)
            image= HERALDO2(fth.propagate(image, x*1e-6, experimental_setup), phi, w, sigma)* np.exp(1j*y)
...
def focusCDI_3(pos,neg, roi, mask=1,phase=0, prop_dist_p=0, prop_dist_n=0,dx=0, dy=0, scale=(0,100), experimental_setup={'ccd_dist':18e-2, 'energy':779.5, 'px_size':20e-6}, operation="-", max_prop_dist=10):
    '''
    Applies a sub-pixel centering, propagation distance and global phase shift.
    Also plots real,image,abs,angle images while you do it
    INPUT:  pos,neg: array, the shifted and masked holograms
    ...
    style = {'description_width': 'initial'}
    fig, axs = plt.subplots(2,2, figsize=(10,10))
    def p(x_p,x_n, y, fx, fy):
        image_p = fth.reconstructCDI(fth.propagate(pos, x_p*1e-6, experimental_setup)* np.exp(1j*y))
        image_n = fth.reconstructCDI(fth.propagate(neg, x_n*1e-6, experimental_setup)* np.exp(1j*y))
        ...
        if operation== "-":
            ...
            image,_=fth.load_both(pos,neg,True)
            image= fth.reconstructCDI(fth.propagate(image, x_p*1e-6, experimental_setup))* np.exp(1j*y)
    ...
    layout = widgets.Layout(width='90%')
    style = {'description_width': 'initial'}
    slider_prop_p = widgets.FloatSlider(min=-max_prop_dist, max=max_prop_dist, step=0.01, value=prop_dist_p, layout=layout,
                                      description='propagation[um]', style=style)
    slider_prop_n = widgets.FloatSlider(min=-max_prop_dist, max=max_prop_dist, step=0.01, value=prop_dist_n, layout=layout,
                                      description='propagation[um]', style=style)
    slider_phase = widgets.FloatSlider(min=-np.pi, max=np.pi, step=0.001, value=phase, layout=layout,
                                       description='phase shift', style=style)
    slider_dx = widgets.FloatSlider(min = -4, max = 4, step = 0.01, value = dx, layout = layout,
    ...
    widgets.interact(p, x_p=slider_prop_p,x_n=slider_prop_n, y=slider_phase, fx = slider_dx, fy = slider_dy)

    return (slider_prop_p,slider_prop_n, slider_phase, slider_dx, slider_dy)
...
def tv_minimize(reco, bs_diam, holo_shape, iterations = 40000, step_size = 1e-3):
    ...
    if type(bs_diam)==int:
        mask = np.ones((x0, y0))
        yy, xx = disk((y0//2, x0//2), bs_diam/2)
        mask[yy, xx] = 0
    else:
        mask=1-bs_diam
